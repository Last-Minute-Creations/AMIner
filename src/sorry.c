...
logBlockBegin("logoGsCreate()");
	...
	TAG_END);
	paletteLoadFromFd(pakFileGetFile(g_pPakFile, "aminer.plt"), s_pVp->pPalette, 1 << s_pVp->ubBpp);
	// Same colors as logo
	...
	tBitMap *pBitMap = bitmapCreateFromFd(pakFileGetFile(g_pPakFile, "lang_select.bm"), 0);
	blitCopy(
